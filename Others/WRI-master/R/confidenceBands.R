...
#' @export
#' @description
#' @details This function computes intrinsic confidence bands for \code{Xpred_df} if \code{type} = 'quantile' and density bands if \code{type} = 'density'. Plots the confidence and/or density bands when \code{figure} = TRUE.
#' @param wass_regress_res an object returned by the \code{wass_regress} function
#' @param Xpred_df k-by-p matrix (or dataframe, or named vector) used for prediction. Note that xpred should have the same column names with Xfit_df used in wass_regress_res
...
#' @param fig_num row of \code{Xpred_df} used for computation of confidence bands. If NULL, then \code{fig_num} is randomly chosen
...
confidenceBands <- function(wass_regress_res, Xpred_df, level = 0.95, delta = 0.01, type = 'density', figure = TRUE, fig_num = NULL){
        ...
        Qobs = wass_regress_res$Ysmooth$Qobs_t
        t = wass_regress_res$t
        m = length(t)
        n = nrow(Qobs)
        ...
        Qfit = quadraticQ(Qfit, wass_regress_res$t)
        Qpred = quadraticQ(Qpred, wass_regress_res$t)
        ...
        if(type %in% c('density', 'both')) {
                ...
                qobs_prime = wass_regress_res$Ysmooth$qobs_prime_t[, index]
        ...
        cdf_list = list(
                ...
                Fpred = matrix(rep(t, k), nrow = k, byrow = TRUE),
                Fsup = Qpred,
                F_lx = t(sapply(1:k, function(i) {approx(x = Q_ux[i, ], y = t, xout = Qpred[i, ], rule = 2)$y})),
                F_ux = t(sapply(1:k, function(i) {approx(x = Q_lx[i, ], y = t, xout = Qpred[i, ], rule = 2)$y})))
        } else {
                quan_list = NULL
                cdf_list = NULL
        }
        ...
        if (figure) {
                ...
                p1 =  ggplot2::ggplot(data = plotdf, ggplot2::aes(x = t)) +
                        ggplot2::geom_line(ggplot2::aes(y = quantile), col = 'slateblue1', size = 0.8) +
                        ggplot2::geom_ribbon(ggplot2::aes(ymin = lower_bound, ymax = upper_bound), fill = 'grey70', alpha = 0.8)+
                ...
                p3 = ggplot2::ggplot(data = plotdf, ggplot2::aes(x = Fsup)) +
                        ggplot2::geom_line(ggplot2::aes(y = Fpred), col = 'slateblue1', size = 0.8) +
                        ggplot2::geom_ribbon(ggplot2::aes(ymin = lower_bound, ymax = upper_bound), fill = 'grey70', alpha = 0.8)+
                ...
                if (type %in% c('density', 'both')) {
                        ...
                        p2 =  ggplot2::ggplot(data = plotdf, ggplot2::aes(x = support)) +
                                ggplot2::geom_ribbon(ggplot2::aes(ymin = lower_bound, ymax = upper_bound), fill = 'grey70', alpha = 0.8)+
                                ggplot2::geom_line(data = plotdf_pdf_full, ggplot2::aes(x = support, y = density), col = 'slateblue1', size = 1.2) +
