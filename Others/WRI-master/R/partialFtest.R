...
partialFtest <- function(full_res, reduced_res, alpha = 0.05) {
        ...
        t = full_res$t
        diff_t = diff(t)
        Qobs = full_res$Ysmooth$Qobs
        Qfit_full = full_res$Qfit

        ### ======================  F test statistic  ====================== ###
        Qmean = colMeans(Qobs)
        F_full = sum(sapply(1:n, function(i) fdapace::trapzRcpp(t, (Qmean - full_res$Qfit[i, ])^2)))
        F_reduced = sum(sapply(1:n, function(i) fdapace::trapzRcpp(t, (Qmean - reduced_res$Qfit[i, ])^2)))
