...
#' \itemize{
#' \item{truncated:}{ asymptotic inference,  p-value is obtained by truncating the infinite summation of eigenvalues into the first K terms.}
...
#' @param bootstrap logical; bootstrap global F test (defailt: FALSE)
...
globalFtest <- function(wass_regress_res, alpha = 0.05, permutation = FALSE, numPermu = 200, bootstrap = FALSE, numBoot = 200) {
        ...
        ### ======================  F test statistic  ====================== ###
        Qmean = colMeans(Qobs)
        Fstat = sum(sapply(1:n, function(i) fdapace::trapzRcpp(t, (Qmean - Qfit[i, ])^2)))

        ### covariance operator C_Q(s, t)
        resi_mat = Qobs - Qfit
        C_Q = cov(resi_mat)
        trace_C_Q =  fdapace::trapzRcpp(t, diag(C_Q))
        ...
        if (permutation) {
                Fstat_vec = sapply(1:numPermu, function(i) {
                        ...
                        fstat = globalFstat(wass_regress_res$xfit, Qobs[index, ], t)
                        return(fstat)
                })
                pvalue_permu = (sum(Fstat_vec > Fstat) + 1)/(numPermu + 1)
                critical_value_permu = NA
                res_df = rbind(res_df, data.frame(
                        method = 'permutation' , statistic = NA,
        ...
        if (bootstrap) {
                ...
                critical_value_boot = NA
                res_df = rbind(res_df, data.frame(
                        method = 'bootstrap' , statistic = NA,
